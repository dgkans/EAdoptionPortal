...
const EditChildProfile = ({ match }) => {
  const url = 'http://3.110.178.10:8080/'
  ...
  const childID = match.params.childId;
    ...
    useEffect(() => {
      const getChild = () => {
        axios.get(`http://3.110.178.10:8080/admin/${childID}`)
          .then((res) => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const res = await axios.put(`http://3.110.178.10:8080/admin/update/${childID}`, childData).then((response) => {});
